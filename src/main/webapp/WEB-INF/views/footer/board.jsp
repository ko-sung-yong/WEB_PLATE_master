...
<td> ${status.count }</td>
<td>  ${g.writer}</td>
<td> <a href="board_cont?gongji_no=${g.gongji_no}&page=${page}">${g.title } </a></td>
<td> ${g.hit} </td>
...
<c:if test="${(empty find_field)&&(empty find_name) }">
 ...
 <c:if test="${page>1}">
 <a href="board?page=${page-1}&find_field=${find_field}&find_name=${find_name}"> [이전]</a>
 </c:if>
 ...
 <c:forEach var="a" begin="${startpage}" end="${endpage}" step="1">
  ...
  <a href="board?page=${a}&find_field=${find_field}&find_name=${find_name}">[${a}]</a> &nbsp;
  </c:if> 
 ...
 <a href="board?page=${page+1}&find_field=${find_field}&find_name=${find_name}">[다음]</a>
